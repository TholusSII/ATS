...
\question{Compléter le schéma bloc de l’asservissement en faisant apparaître la régulation de l’inclinaison.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\question{Calculer l’inclinaison $\Psi(t)$ du châssis en régime permanent, lorsque la
...
\question{Démontrer que ce correcteur permet de satisfaire le cahier des charges vis-à-vis de l’écart en régime permanent pour une perturbation en échelon.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\question{Tracer les diagrammes de Bode asymptotiques et réels (allure uniquement)
...
\question{Déterminer $\omega_c$ telle que la marge de la FTBO soit $M_{\varphi}=45\degres$. En
...
\question{Déterminer alors $K_i$ tel que $\omega_c$ soit effectivement la pulsation de coupure
...
%
%
%\question{Analyser la stabilité du système d’entrée $u(t)$ et de sortie $\psi(t)$ en étudiant la fonction de transfert $F_1(p)=\dfrac{\Psi(p)}{U(p)}$.}% Pouvait-on s'attendre à ce résultat ?}
%\ifprof
%\begin{corrige}
%\end{corrige}
%\else
...
%
%
%\question{Dans le cas où $\alpha = 0$, déterminer, en fonction de $K_S$, $k_p$, $k_v$ et $\omega_1$ la
...
%
%
%\question{Déterminer les conditions sur $k_v$ et sur $k_p$ pour que le système soit stable.}
%\ifprof
%\begin{corrige}
%\end{corrige}
%\else
...
%
%
%\question{Déterminer les expressions de $K_2$, $\xi$ et $\omega_0$.}
%\ifprof
%\begin{corrige}
%\end{corrige}
%\else
...
%
%
%\question{Déterminer les valeurs de $k_p$ et de $k_v$ telles que le temps de réponse à 5\% soit minimal.}
%\ifprof
%\begin{corrige}
%\end{corrige}
%\else
