...
\ifprof
% Le bloc listings historique provoque sous certaines versions de MiKTeX
% une erreur T1 au moment de \begin{lstlisting}. Un verbatim standard
% suffit ici, puisque le code doit seulement être affiché.
\begin{verbatim}
def plot_debit5p():
    plt.cla()
    ...
    # Calcul de la vitesse instantanee des pistons.
    ...
    # 5 courbes de debit decalees d'un cinquieme de tour
    ...
    plt.xlabel("$\\theta$ (deg)")
    plt.ylabel("Debit instantane $m^3s^{-1}$")
    
    # On conserve que les valeurs positives (debit)
    ...
    # Le debit instantane est la somme des contributions
