...
\question{Montrer que le couple des deux électro-aimants permet d'assurer l'effort
...
\question{Déterminer l'expression de l'effort $F$ en fonction de $x$, de $I$ et de paramètres
...
\question{On note désormais $\vect{F_T}=F_t\vect{x_0}$ l'effort total exercé par les deux couples
...
\question{Appliquer le Principe Fondamental de la Dynamique à la masse ponctuelle
...
\question{Déterminer la fonction de transfert $H_{\text{PM}}=\dfrac{X(p)}{U(p)}$. Le système est-il stable ? Justifier.}
\ifprof
\begin{corrige}
\end{corrige}
\else
